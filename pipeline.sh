...
grep -Exclude '^\d|^[A-Z]|All' $analysis_dir/tables/results.txt > $analysis_dir/tables/toplines.txt
grep -Exclude '^\d|^[A-Z]|\*|All' $analysis_dir/tables/results.txt > $analysis_dir/tables/significant.txt
